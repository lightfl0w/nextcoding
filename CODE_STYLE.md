...
4. **注释用 TSDoc**。公开 API（导出的 hooks、函数、类型）使用标准 TSDoc 语法：首行摘要 + 块级标签（`@param`、`@returns` 等）分行书写。
   - 注释要**短**、直白；禁止复述代码、禁止套话（如「该函数用于…」）。
   - `@param` 描述用 `-` 连接；含义显而易见的参数可省略标签。
   - 功能性指令（`@ts-expect-error`、`biome-ignore`、`/// <reference>` 等）不是注释，保留。
